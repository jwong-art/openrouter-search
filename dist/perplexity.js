...
const perplexitySearchParams = Type.Object({
    ...
    search_context_size: Type.Optional(Type.Union([Type.Literal("low"), Type.Literal("medium"), Type.Literal("high")], {
        ...
        description: "How much web search context to retrieve before answering. Defaults to medium. Use high for deeper research and low for cost-efficient simple lookups.",
    })),
...
async function callPerplexitySearch(params, config, signal) {
    ...
    if (!response)
        throw new Error("OpenRouter Perplexity Search failed: no response returned.");
    if (!response.ok) {
...
const toolDescription = "Search the web with OpenRouter Chat Completions model=perplexity/sonar-pro-search. " +
...
export function createPerplexitySearchTool(tool) {
    return tool({
        ...
        factory: (context) => ({
            ...
            execute: async (_toolCallId, params, signal) => {
                const result = await callPerplexitySearch(params, context.config, signal);
                return {
                    content: [
                        {
                            type: "text",
