...
function registerTools() {
  ...
  return registered.map((item) =>
    typeof item === "function" ? item({ config: { timeout_ms: 180000 }, api: {}, toolContext: {} }) : item,
  ) as any[];
}
...
describe("openrouter-search", () => {
  ...
  it("declares the combined OpenRouter search plugin metadata", () => {
    ...
    const xParameters = metadata?.tools.find((tool) => tool.name === "openrouter_x_search")?.parameters as any;
  ...
  it("rejects mutually exclusive X handle filters before calling OpenRouter", async () => {
    ...
    ).rejects.toThrow("allowed_x_handles and excluded_x_handles are mutually exclusive");
